...
class TestCallSignProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config.get_input.return_value = {
            "folder": "test_input",
            "files": {
                "two_letter_words": "2l.txt",
                "three_letter_words": "3l.txt"
            }
        }
    ...
    def test_process_callsigns(
        self, mock_write, mock_sort, mock_include_exclude,
        mock_process_options, mock_match, mock_load
    ):
    ...
    def test_invalid_match_option(self):
        ...
        processor = CallSignsProcessor(
            self.config, "us", "phonetic", "pair", ["cw"],
            ["invalid"], [], [], "asc"
        )
    ...
    def test_process_options_all_option(self, mock_write):
        ...
        with patch.object(NumberOfLetters, 'get_supported_number_of_letters', 
                          return_value=supported_letters):
    ...
    def test_process_callsigns_exclude_only(
        self, mock_write, mock_sort, mock_include_exclude, 
        mock_process_options, mock_load
    ):
    ...
    @patch.object(CallSignsProcessor, 'process_match_option')
    @patch('hrt.processors.callsign_processor.write_output')
    def test_process_callsigns_with_3l_match(
        self, mock_write, mock_match, mock_load
    ):
    ...
    @patch.object(CallSignsProcessor, 'load_callsigns')
    @patch('hrt.processors.callsign_processor.write_output')
    def test_process_callsigns_with_invalid_match(
        self, mock_write, mock_load
    ):
    ...
    def test_process_callsigns_invalid_word_length(self, mock_load):
        ...
        processor = CallSignsProcessor(
            self.config, "us", "phonetic", "pair", [],
            ["4l"], [], [], "asc"  # 4l is not a valid length
        )
    ...
    def test_process_callsigns_empty_set(
        ...
        mock_process_options.side_effect = [set(), set()]  # Empty results for include/exclude options
    ...
    def test_process_callsigns_include_only(
        ...
        processor = CallSignsProcessor(
            self.config, "us", "phonetic", "pair", [],
            [], ["END"], [], "asc"  # Note: empty exclude_options
        )
    ...
    def test_process_callsigns_with_cw_rank(
        ...
        processor = CallSignsProcessor(
            self.config, "us", "phonetic", "pair", [RankBy.CW_WEIGHT.value],
            [], ["END"], [], "asc"
        )
